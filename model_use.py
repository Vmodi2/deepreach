...
if not os.path.exists('air3d'):
    os.mkdir('air3d')

model = modules.SingleBVPNet(in_features=4, out_features=1, final_layer_factor=1.,hidden_features=512, num_hidden_layers=3)
model.cuda()
model.load_state_dict(torch.load('../logs/experiment_3/checkpoints/model_final.pth'))
