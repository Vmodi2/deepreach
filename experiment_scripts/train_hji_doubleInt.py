...
# import wandb
# wandb.init(project="deepreach-double-integrator")
...
p.add_argument('--acc_max', type=float, default=1.1, required=False, help='Maximum acceleration of car')
...
p.add_argument('--CBVF', default=False, action='store_true', help='Enable/disable CBVF formulation')
opt = p.parse_args()
...
dataset = dataio.ReachabilityDoubleIntegrator(numpoints=65000, collisionR=opt.collisionR, acc_max=opt.acc_max, 
...
# Define the loss
loss_fn = loss_functions.initialize_hji_doubleintegrator(dataset, opt.minWith, CBVF=opt.CBVF)
...
def val_fn(model, ckpt_dir, epoch):
  ...
  # Create a figure
  fig = plt.figure(figsize=(5, 5*num_times))
